...
public class CommandTools {
	...
	public static String convertSwapVehicleInfo(String msg,String vehicleInfo)
		...
		try{
			...
			if(registrationNo!="" || registrationNoColor!=""){
				str.append("车牌号码：" + registrationNo)
				.append("，车牌颜色：" + registrationNoColor);
			}
			str.append(" "+msgArr[2]);
			return str.toString();
		}catch(Exception e){
	...
	public static String convertChaGuan(String msg, String commandCode)
		...
		try{
			...
			StringBuffer str = new StringBuffer("");
			String[] msgArr = msg.split(",");
			str.append("查岗对象类型：" + Tools.flatMap.get(msgArr[2]))
			.append(",查岗对象ID：" + msgArr[3])
			.append(",查岗信息ID：" + msgArr[4])
			.append(",查岗信息内容：" + msgArr[5])
			.append("@@@" + msgArr[2])// 查岗对象类型
	...
	public static String convertPost(String msg, String commandCode)
		...
		try{
			...
			StringBuffer str = new StringBuffer("");
			String[] msgArr = msg.split(",");
			str.append("报文对象类型：" + Tools.flatPlatMap.get(msgArr[2]))
			.append(",报文对象ID：" + msgArr[3])
			.append(",报文信息ID：" + msgArr[4])
			.append(",报文信息内容：" + msgArr[5])
			.append("@@@");
			return str.toString();
		}catch(Exception e){
	...
	public static String convertOverseeing(String msg,String vehicleInfo) throws Exception {
		try{
			...
			str.append("通讯类型：" + msgArr[0])
				.append("，车牌号码：" + registrationNo)
				.append("，车牌颜色：" + registrationNoColor)
				.append("，报警信息来源：" + Tools.alarmSourceStrMap.get(msgArr[2]))
				.append("，报警类型：" + Tools.alarmTypeMap.get(msgArr[3]))
				.append("，报警时间：" + msgArr[4]).append(",督办截止时间:" + msgArr[6])
				.append("，督办级别：" + Tools.overseeingMap.get(msgArr[7]))
				.append("，督办人：" + msgArr[8]).append(",督办联系电话:" + msgArr[9])
				.append("，督办联系电子邮件：" + msgArr[10])
	...
	public static String convertAlarmAdvance(String msg,String vehicleInfo) throws Exception {
		try{
			...
			str.append("，报警信息来源：" + Tools.alarmSourceStrMap.get(msgArr[2]))
			   .append("，报警类型：" + Tools.alarmTypeMap.get(msgArr[3]))
			   .append("，报警时间：" + msgArr[4])
	...
	public static String convertUpCustomAnswer(String msg,String commandCode){
		try{
			...
			String[] msgArr = msg.split(",");
			StringBuffer str = new StringBuffer("");
			str.append("上级下发应答类型："+Tools.upAnswerMap.get(msgArr[2])).append(",描述："+msgArr[3]);
			return str.toString();
		}catch(Exception e){
