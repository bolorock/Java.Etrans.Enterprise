...
public class ReceivedMessage implements Runnable,MscSocketListener {
	 ...
	 *区域接收队列
	 */
	private AreaInfoQueue areaInfoQueue;
	 ...
	 * scpt_19_OnlineCars        = 19,  //在线车辆数 压力测试用
	 * scpt_20_Zone              = 20   //区域信息
	 * 
     * )##NOOP
	 * @throws Exception 
	 */
	private String[] receiveAry;
	...
	public void MessageHandle(String receivedMessage) throws Exception {
		try {
			...
			if(receivedMessage!=null && receivedMessage.length()>0){
				receiveAry = receivedMessage.split(",");				
				if(receiveAry.length>1){
					commandType = receiveAry[1];
					if(commandType.equals("5")) { 
						gpsInfoQueue.produce(receivedMessage);
					}else if(commandType.equals("12")){
						alarmQueue.produce(receivedMessage+","+System.currentTimeMillis());
					}else if(commandType.equals("8")|| commandType.equals("10")){
						commandResultQueue.produce(receivedMessage);
					}else if (commandType.equals("6") || commandType.equals("13")) { 
						businessQueue.produce(receivedMessage.toString());
					}else if(commandType.equals("18")){ //附加信息
						affixationQueue.produce(receivedMessage.toString());
					}else if(commandType.equals("19")){//压力测试用
						
					}else if(commandType.equals("20")){ //区域信息
						areaInfoQueue.produce(receivedMessage.toString());
					}
				}
			}
