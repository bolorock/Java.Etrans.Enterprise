...
public class OilServices {
	...
	public PageBean getOilInfo(Map setParamsMap, SessionUser sessionUser)
			throws Exception {
		...
		try {
			rows = this.ibatisServices.queryForList(Map.class, "getOilInfoSQL",
					setParamsMap);
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 */
	@SuppressWarnings( { "unchecked" })
	public List<Map<String, Object>> getOilInfoExl(Map setParamsMap,
...
//		setParamsMap.put("@workunitid", String.valueOf(sessionUser.getWorkUnitID()));
		...
		List<Map<String, Object>> rows = this.ibatisServices.queryForList(
				Map.class, "getOilInfoSQL", setParamsMap);
		return rows;
