...
@Service
public class WorkUnitOilServices {
	@Autowired
	...
	public PageBean getWorkUnitOilInfo(Map setParamsMap, SessionUser sessionUser)
			throws Exception {
		...
		try {
			rows = this.ibatisServices.queryForList(Map.class, "getWorkUnitOilInfoSQL",
					setParamsMap);
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 */
	@SuppressWarnings( { "unchecked" })
	public List<Map<String, Object>> getWorkUnitOilInfoExl(Map setParamsMap,
		...
		List<Map<String, Object>> rows = this.ibatisServices.queryForList(
				Map.class, "getWorkUnitOilInfoSQL", setParamsMap);
		return rows;
