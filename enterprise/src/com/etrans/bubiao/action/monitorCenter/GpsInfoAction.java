...
//		}
	 ...
	 * （实时报警，历史报警中使用）
	 * @author yangzhen
	 * @since Create on 2012-08-25
	 * 
	 */
	@Action(value = "getBaiduAddress")
	public void getBaiduAddress(){
		 String lnglat=getParameter("lnglat");
		 String[] SHlngArray = Tools.getRealngLatBaidu(lnglat);//偏移后的经纬度
		 String SHlngLat=SHlngArray[1]+",%20"+SHlngArray[0];
		  try {
		 StringBuffer urlBuffer=new StringBuffer();
			urlBuffer.append("http://api.map.baidu.com/geocoder")
			         .append("?")
			         .append("output=json&location=")
			         .append(SHlngLat)
			         .append("&key=e642cd2e1957adba831844b06766f4d5");
		   Response resulResponse=HttpClient.getAddress(urlBuffer.toString());
		   this.renderJSON(resulResponse.asString());
	...
	/**
	 * 
	 * 百度取中文地址http接口--所有取地址的都用这个接口
	 * 描述：取中文地址,经纬度经过偏移
	 * (订阅，轨迹回放中使用)
	 * @author yangzhen
	 * @since Create on 2013-08-26
	 * @version 
	 */
	@Action(value = "getBaiduAddressSH")
	public void getBaiduAddressSH(){
		 String lnglat=getParameter("lnglat");
		 String[] SHlngArray=lnglat.split(",");
		 String SHlngLat=SHlngArray[1]+",%20"+SHlngArray[0];
		  try {
			    StringBuffer urlBuffer=new StringBuffer();
				urlBuffer.append("http://api.map.baidu.com/geocoder")
