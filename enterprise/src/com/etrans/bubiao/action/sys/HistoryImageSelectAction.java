...
public class HistoryImageSelectAction extends BaseAction {
	...
	public void historyImageSelectList() {
		try {
			...
			LogUtil.insertLog(LogActionTypes.READ, "成功", "历史图片查询", "", "历史图片查询");
		} catch (Exception e) {
			LogUtil.insertLog(LogActionTypes.READ, "失败", "历史图片查询", "", "历史图片查询");
			e.printStackTrace();
		}
	}
	...
	public void exportHistoryImageSelect(){
		...
		try {
			...
			HSSFWorkbook wb=null;
				...
				columnArray[6]="picture";
				
	             
				exportExl("historyImageSelectList", titleArray, columnArray, list,(Integer)params.get("picWidth"),(Integer)params.get("picHeight"));
				LogUtil.insertLog(LogActionTypes.READ, "成功", "历史图片导出", "", "历史图片导出");
			
			
		} catch (Exception e) {
			LogUtil.insertLog(LogActionTypes.READ, "失败", "历史图片导出", "", "历史图片导出");
			e.printStackTrace();
		}
	}
