...
//						
//						//取客户端传来的车牌号码
//						registrationNo = new String(paramBean.getVehicleNoList().get(i).getBytes("ISO-8859-1"),"gbk");
//						param.put("RegistrationNo", new String(paramBean.getVehicleNoList().get(i).getBytes("ISO-8859-1"),"gbk"));
//						
//						//验证车牌号码 是否属于这个用户
...
//							
//							String gpsInfo=this.commandRepository.getGpsInfo(String.valueOf(map.get("id")));
...
//					param.put("RegistrationNo", new String(paramBean.getVehicleNo().getBytes("ISO-8859-1"),"gbk").replace("?", ""));
//					
//					//验证车牌号码 是否属于这个用户
//					lists=ibatisServices.queryForList(Map.class,"getRegistrationNoSQL",param);
//					
...
//	}
	...
	/**
	 *http://localhost:80/enterprise/httpService/ETGetVehicleGpsInfos.action?jsonParam={"userID":"51","ticket":"1358835197986-5021" ,vehicleIdList:["301","302"]} 
	 *根据车牌取用户下所有车辆的最新定位信息
	 */
	...
	public void ETGetVehicleGpsInfos() throws Exception 
	{
		...
		try{
			...
			if(flag && paramLs.size()>0){
				...
				List<HashMap<String,String>> ls = this.ibatisServices.findIbatisList("querySimpleVehicleInfoByIdSQL",map);
				
				for(HashMap<String,String> paramMap:ls){
					
					GpsInfo info = this.commandRepository.getVehicleGpsInfo(String.valueOf(paramMap.get("vehicleID")),"0");
	...
	/**
	 *http://localhost:8080/enterprise/httpService/ETGetHistoryGpsInfo.action?jsonParam={"userID":"51","ticket":"1359099191954-1548" ,"vehicleId":"51920", "startDate":"2012-10-18 10:00:00", "endDate":"2012-10-18 12:00:00"}
	 *1.6.11	获取某辆车某段时间内的历史轨迹数据
	 */
	@Action(value = "ETGetHistoryGpsInfo")
	public void ETGetHistoryGpsInfo() throws Exception 
	{
		...
		try{
			...
			if(flag){
				...
				List<HashMap<String, Object>> historyGpsInfoList =this.historyGpsInfoServices.getHistoryGpsInfo(paramMap);
				Map<String,Object> paramsMap = new HashMap<String,Object>();
				paramsMap.put("vehicleID", paramBean.getVehicleId());
				List<HashMap<String,String>> registrationNOMap=this.historyGpsInfoServices.getUserVehicleIDByRegistrationNO(paramsMap);
				
				for(Map<String,Object> infoMap:historyGpsInfoList){
					double lonDouble=(Double)infoMap.get("longitude");
					double latDouble=(Double)infoMap.get("latitude");
					
					infoMap.put("longitude", String.valueOf(lonDouble/1000000));
					infoMap.put("latitude", String.valueOf(latDouble/1000000));
					
					if(registrationNOMap != null && registrationNOMap.size()>0){
						Map<String,String> map=registrationNOMap.get(0);
						infoMap.put("registrationNO", map.get("registrationNO"));
						infoMap.put("registrationNOColor", map.get("registrationNOColor"));
					}
				}
				String str = JSONUtil.toJson(historyGpsInfoList);
				result.setData(str);
				/****************返回结果********END****************/
