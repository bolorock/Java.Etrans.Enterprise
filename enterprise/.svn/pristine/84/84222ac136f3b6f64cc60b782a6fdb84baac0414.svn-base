...
$(function() {
	...
	load(); 
		...
		$("#analyseMileagePerDayList").flexigrid( {
			url : 'analyse/driving/findAnalyseMileagePerDayList.action',
			dataType : 'json',
			params : params,
			colModel : [ 
			{
...
*/
function toSearch(){
	$("#analyseMileagePerDayList").hide();
	//查询参数
	var params=getparams();
	// 重置表格的某些参数
	$("#analyseMileagePerDayList").flexOptions({
			newp : 1,// 设置起始页
			params : params// 设置查询参数
		}).flexReload();// 重新加载
	$("#analyseMileagePerDayList").show();
}
...
function toExportExl() {
	exportExl('analyseMileagePerDayList', 'analyse/driving/analyseMileagePerDayExport.action');
}
