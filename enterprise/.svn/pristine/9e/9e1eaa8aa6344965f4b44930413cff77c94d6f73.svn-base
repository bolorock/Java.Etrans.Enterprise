$(function() {
		...
		$("#areaAlarmConfigList").flexigrid( {
			url : 'monitorCenter/findAreaAlarmConfigList.action',
			dataType : 'json',
			params : params,
			colModel : [ 
			{
...
function getHandleColumn(id){
	...
	var deleteStr = "";
	if(resources!=null){
		 if(resources.indexOf("|getAreaAlarmConfigById|")!=-1){
			 
			 editStr = '<a href="javascript:void(0)" title="查看"  onclick="doView(' + id + ')">查看</a>';
		 }
		 
		 if(resources.indexOf("|deleteAreaAlarmConfig|")!=-1){
...
function toSearch(){
	var params= getparams();
	$("#areaAlarmConfigList").flexOptions({
			newp : 1,
			params : params
		}).flexReload();
}
...
function toCreate(){
	...
	//初始化区域名称
	initAjaxSelect("areaId","sys/getCustomArea.action",0);
	//加载检测时间类型列表
...
function ConfigAdd(){
	...
	if(flag){
		...
		$.ajax({
		    type : "POST",
		    url : "monitorCenter/createAreaAlarmConfig.action",
		    data : params,
		    dataType : "JSON",
		    success : function(data) {
		    	if(data.code == '1'){
		    		 hide();
		    		$("#areaAlarmConfigList").flexReload();
		    	}else{
		    		showError();
		    	}
		    },
...
function doView(id){
	...
	if(id != null && id != ''){
		...
		$.post("monitorCenter/getAreaAlarmConfigById.action", {id : id}, function(data){
			if(data.code == '1'){
				...
				//相关区域点
				var areaPointList = config.areaPoints;
				var areaStr = "";
				areaStr += "<ul>";
				if(areaPointList && areaPointList.length>0){
					for ( var i = 0; i < areaPointList.length ; i++) {
						areaStr += "<li>" + areaPointList[i].Longitude + "," + areaPointList[i].Latitude + "</li>";
					}
				}
				areaStr += "</ul>";
...
function doDelete(id){
	if (id != null) {
		if (!confirm("确定删除此区域报警设置?")) {
			return false;
		} else {
				$.ajax({
				    type : "POST",
				    url : "monitorCenter/deleteAreaAlarmConfig.action",
				    data : {id:id.toString()},
				    dataType : "JSON",
				    success : function(data) {
				    	if(data.code == '1'){
