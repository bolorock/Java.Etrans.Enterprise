...
$(function() {
		...
		$("#findVehicleOSTimesStatList").datagrid( {
			url : 'query/stat/findvehicleOSTimesStatList.action',
...
//	
	...
	//载入并显示第一页的记录
	$('#findVehicleOSTimesStatList').datagrid('load',params);
...
function setPageControl(){
	...
	/**分页组件datagrid**/
	var p = $('#findVehicleOSTimesStatList').datagrid('getPager');  
    $(p).pagination({  
...
function toExportExl(){
	var options= $('#findVehicleOSTimesStatList').datagrid('options');
...
//	var week=34;
	...
	exportExlEasyui('findVehicleOSTimesStatList', 'query/stat/doVehicleOSTimesStatListExportExl.action?workUnitName='+workUnitName+'&year='+year+
			'&month='+month+'&day='+day+'&week='+week+'&registrationNo='+registrationNo+'&page='+page+'&rows='+rows);
