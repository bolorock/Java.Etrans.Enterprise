Poly_line = Class.create(); 
Poly_line.prototype = Object.extend(new Abstract.OverLayer(), {
    
    initialize: function(points, color, size,name){
	 	this.id = Util.createUniqueID('Over_Poly_line_');
	 	this.name=name;
        this.points = points;
        this.color = color;                
        this.size = size;
    ...
    setToMap: function(mapDiv, model, overLayerDiv){
		...
		lines.push('" style="position:relative; left:2px;top:-3px"; strokecolor="'+this.color+'" strokeweight="'+this.size+'"/>');
        if(overLayerDiv){
