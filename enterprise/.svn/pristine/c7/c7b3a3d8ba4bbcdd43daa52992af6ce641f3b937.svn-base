...
$(function() {
		...
		$("#drivingDOTGZSJWList").flexigrid( {
			url : 'analyse/driving/findDrivingDOTGZSJWList.action',
			dataType : 'json',
			params : params,
			colModel : [ 
			{
...
*/
function toSearch(){
	$("#drivingDOTGZSJWList").hide();
	//查询参数
	var params=getparams();
	// 重置表格的某些参数
	$("#drivingDOTGZSJWList").flexOptions({
			newp : 1,// 设置起始页
			params : params// 设置查询参数
		}).flexReload();// 重新加载
	$("#drivingDOTGZSJWList").show();
}
...
function toExportExl() {
	exportExl('drivingDOTGZSJWList', 'analyse/driving/drivingDOTGZSJWExport.action');
}
