...
//				checkedSelectByValue("UpdateWorkUnitID",data.workUnitId);
				...
				checkedSelectByValue("Status1",data.status);
...
function doUpdate(){
	...
	if(flag){
		...
		$.ajax({
		    ...
		    data : {id:id,name:name,status:status,abbre:shortName,workUnitId:workUnitId},
		    dataType : "JSON",
		    success : function(data) {
		    	if(data.code == '0'){
		    		hideEdit();
...
function openAddDialog(id) {
	$("#dialogFrame").attr("src", "basicBlue/role/configRole.jsp?roleId="+id);
	openDialog("dialogs",860, 460, '权限管理');
}


function openAddTADialog(id) {
	$("#dialogTAFrame").attr("src", "analyse/configRole.jsp?roleId="+id);
	openDialog("dialogTAs",320, 460, 'TA权限管理');
}
...
function getHandleColumn(ID){
	...
	var authorityLinkTAStr = "";
	var editStr = "";
	var deleteStr = "";
	if(resources!=null){
		if(resources.indexOf("|createRole|")!=-1){
			//authorityLinkTAStr = '<a href="javascript:void(0)" title="TA权限管理" onclick="openAddTADialog(' + ID + ')">TA权限管理</a>';
	...
	return authorityLinkTAStr + '&nbsp;&nbsp;' +authorityLinkStr + '&nbsp;&nbsp;' + editStr+ '&nbsp;&nbsp;' + deleteStr;
}
