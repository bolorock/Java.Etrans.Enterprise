...
$(function() {
	...
	load();
		...
		$("#drivingRoadOverSpeedList").flexigrid( {
			url : 'analyse/driving/findDrivingRoadOverSpeedList.action',
			dataType : 'json',
			params : params,
			colModel : [ 
			{
...
*/
function toSearch(){
	$("#drivingRoadOverSpeedList").hide();
	//查询参数
	var params=getparams();
	// 重置表格的某些参数
	$("#drivingRoadOverSpeedList").flexOptions({
			newp : 1,// 设置起始页
			params : params// 设置查询参数
		}).flexReload();// 重新加载
	$("#drivingRoadOverSpeedList").show();
}
...
function toExportExl() {
	exportExl('drivingRoadOverSpeedList', 'analyse/driving/drivingRoadOverSpeedExport.action');
}
