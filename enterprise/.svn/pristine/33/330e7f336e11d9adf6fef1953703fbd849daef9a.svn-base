...
@Service
public class VehicleEffectiveUploadService {
	@Autowired
	...
	public PageBean getVehicleEffectiveUpload(String queryJSON,
...
//		setParamsMap.put("@workunitid", String.valueOf(sessionUser.getWorkUnitID()));
		...
		try {
			rows = this.ibatisServices.queryForList(Map.class,
					"getVehicleEffectiveUploadSQL", setParamsMap);
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 */
	@SuppressWarnings( { "unchecked" })
	public List<Map<String, Object>> getVehicleEffectiveUploadExportExl(
		...
		List<Map<String, Object>> rows = this.ibatisServices.queryForList(
				Map.class, "getVehicleEffectiveUploadSQL", setParamsMap);
		return rows;
