...
$(function() {
	 ...
	 initWeek2Date("slBeginWeek");
		...
		$("#findVehicleEffectiveUploadList").flexigrid( {
			url : 'query/stat/findVehicleEffectiveUpload.action',
			dataType : 'json',
			params : getparams(),
			colModel : [ 
			{
				display : '车牌号码',//表头registrationNo
...
//	$("#uplinePercentChart").hide();
	...
	// 重置表格的某些参数
	$("#findVehicleEffectiveUploadList").flexOptions({
			newp : 1,// 设置起始页
...
function toExportExl() {
	exportExl('findVehicleEffectiveUploadList', 'query/stat/VehicleEffectiveUploadExportExl.action');
}
