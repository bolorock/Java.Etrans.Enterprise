...
function load(terminalKindIDS){
	var mainFrame = window.dialogArguments; // 接收Left页面传来的参数
	terminalKindID = terminalKindIDS;//mainFrame[0];
	//按钮绑定点击事件
    getFunctionCode();
...
function getPubCommandCode(){
	...
	var src="command/batch/batchGbIndex.jsp?commandKindID="+commandKindID+"&commandId="+commandId+"&vehicleId="+vehicleId;
