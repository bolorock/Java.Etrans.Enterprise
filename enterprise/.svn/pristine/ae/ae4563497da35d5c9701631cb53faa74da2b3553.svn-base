...
(function($) {
  ...
  EditableSelect.prototype = {
    ...
    init: function(select, settings) {
      ...
      this.text.attr('id', id + '_hidden_text');
    ...
    */
    duplicateOptions: function() {
      ...
      options.each(function() {
    	var optiontext = $.trim($(this).text().toString());
    	optiontext = optiontext.replace(new RegExp("\n","gm"), "");
        if($(this).attr('selected')) {
          context.text.val(optiontext);
          context.current_value = $(this).val();
          context.current_text = optiontext;
        };
    ...
    pickListItem: function(list_item) {
      if(list_item.length) {
    	var itemtext = $.trim(list_item.text().toString());
    	itemtext = itemtext.replace(new RegExp("\n","gm"), "");
        this.text.val(itemtext);
        //this.text.val(list_item.text());
        var itemvalue = list_item.attr("value");
        this.current_value = itemvalue;
        this.current_text = this.text.val();
        //this.select.attr("value",this.current_value);
        /**
        var options = this.select.find('option');
        options.each(function() {
        	var optiontext = $.trim($(this).text().toString());
