 ...
 */
function reSetAddFormTA(divID,checkTimeValue,checkTimeBegin,checkTimeBegin_go,checkTimeEnd,checkTimeEnd_go){
	$("#" + divID + " input:not(:button)").each(function(i){
		var valueID=$(this).attr("id");
		//开始时间控件id和结束时间控件id都不为空
		if((checkTimeBegin!=null&&checkTimeBegin!="")||(checkTimeEnd!=null&&checkTimeEnd!="")){
			if((valueID==checkTimeBegin)||(valueID==checkTimeEnd)){
				//设置三个控件同时不可用
				jspControlYesOrNo(checkTimeValue,checkTimeBegin,checkTimeBegin_go,checkTimeEnd,checkTimeEnd_go,"no");
				//设置默认时间【initTime_Value在initSelects.js文件里面】
...
//例子
//{setParam={name=部标578J}, whereParam={id=6}, tableName=Pub_TerminalKind, orderParam={}}
//	 
//	 var setParam = new QueryParam();
//	 setParam.put("@CheckingNo","");
//	 setParam.put("@CheckTime","''");
//	 setParam.put("@CheckContent","5e");
//	 setParam.put("@IsResult","1");
//	 
//	 var mapParams = new QueryParam();
//	 mapParams.put("@setParam",setParam.toJSON());
//	 mapParams.put("@tableName","Ent_HighLevelPatrol_Log");
//	 
...
function toEditSucc(recordInfo){
	if(recordInfo!=null){
		if(recordInfo.length > 0){
			$("#checkTimeTypeId").val(recordInfo[0]["checkTimeTypeId"]);
//			var tempId = document.getElementById("checkTimeTypeId").selectedIndex;
//			$("#checkTimeTypeId").attr("selectedIndex",tempId);
			dateTypeIDSelOnchange_TA('checkTimeTypeId','checkTimeValue','checkTimeBegin','checkTimeBegin_go','checkTimeEnd','checkTimeEnd_go');
			...
			//alert(recordInfo[0][DXMTypeName]);
			
			if(DXMTypeName!=""){
				//点线面下拉框赋值，并且不可用
				initSelectDXM(DXMTypeName,recordInfo[0][DXMTypeName]);
			}
