...
$(function() {
		...
		$("#drivingOverSpeedPolyList").flexigrid( {
			url : 'analyse/driving/findDrivingOverSpeedPolyList.action',
			dataType : 'json',
			params : params,
			colModel : [ 
			{
...
*/
function toSearch(){
	$("#drivingOverSpeedPolyList").hide();
	//查询参数
	var params=getparams();
	// 重置表格的某些参数
	$("#drivingOverSpeedPolyList").flexOptions({
			newp : 1,// 设置起始页
			params : params// 设置查询参数
		}).flexReload();// 重新加载
	$("#drivingOverSpeedPolyList").show();
}
...
function toExportExl() {
	exportExl('drivingOverSpeedPolyList', 'analyse/driving/drivingOverSpeedPolyExport.action');
}
