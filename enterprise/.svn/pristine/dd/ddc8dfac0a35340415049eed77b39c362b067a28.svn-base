...
$(function() {
	...
	load();
		...
		$("#drivingDriverOvertimeList").flexigrid( {
			url : 'analyse/driving/findDrivingDriverOvertimeList.action',
			dataType : 'json',
			params : params,
			colModel : [ 
			{
				...
				display : '门限值(KM或分)',
				name : 'BounceOverValue',
				width :150,
				sortable : true,
				align : 'left'
			}, {
				display : '记录值(KM或分)',
				name : 'RecordOverValue',
				width :150,
				sortable : true,
				align : 'left'
			}, {
...
*/
function toSearch(){
	$("#drivingDriverOvertimeList").hide();
	//查询参数
	var params=getparams();
	// 重置表格的某些参数
	$("#drivingDriverOvertimeList").flexOptions({
			newp : 1,// 设置起始页
			params : params// 设置查询参数
		}).flexReload();// 重新加载
	$("#drivingDriverOvertimeList").show();
}
...
function toExportExl() {                                      
	exportExl('drivingDriverOvertimeList', 'analyse/driving/drivingDriverOvertimeExport.action');
}
