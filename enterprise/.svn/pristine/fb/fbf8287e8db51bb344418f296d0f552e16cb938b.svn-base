		...
		try {
			for (String vehicleId : vehicleIdStr.split(",")) {
				   vehicleId = StringUtils.deleteWhitespace(vehicleId);
					...
					String areaInfo = this.getAreaInfo(vehicleId);//区域信息
					/**
					...
					if (null != gpsInfoMessage) {									
						...
						// 判断是否有司机信息g
						String stateStr = "";// 状态
						stateStr = State.getTermianlState(gpsInfoMessageArray[17]);//gps状态|报警状态  非压力测试开启
//						stateStr = gpsInfoMessageArray[17];//gps状态|报警状态   压力测试					    
						...
						if((nowTime-gpsTime)<=600000){//10分钟内有轨迹   正式使用
						//if((nowTime-gpsTime)<=60000){//10分钟内有轨迹   过检
							info.setOnLine(true);
						}else{
...
//						info.setsHlon(gpsInfoMessageArray[3]);//偏移后经度
						...
						info.setSh(String.valueOf(Float.valueOf(gpsInfoMessageArray[14])/10));//记录仪里程
						...
						if(affixationBean!=null){
							...
							//附加信息整体字符串
							info.setFjInfo(affixationBean.getFjInfo()==null?"":affixationBean.getFjInfo().toString());
						}
						
						/**区域信息**/
						if(areaInfo!=null){
							info.setAreaInfo(areaInfo);
						}
...
//					}
	 ...
	 * 区域信息数据处理线程
	 * */
	private void areaInfoHandleThread() {
		taskExecutor.execute(new AreaInfoHandleThread(areaInfoHashMap, areaInfoQueue));
	}
	...
	private void alarmHandleThread() {
		taskExecutor.execute(alarmHandleThread = new AlarmHandleThread(alarmHashMap,alarmQueue));
	}
	/**
	 * 指令回复数据处理线程
	 * */
	...
	private void getReceivedMessage() {
		try {
			taskExecutor.execute(receivedMessage = new ReceivedMessage(socketTemplate, gpsInfoQueue, businessQueue,commandResultQueue,alarmQueue,affixationQueue,areaInfoQueue));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public String getGpsInfo(String vehicleId) throws Exception {
		// 压力测试 star
		// 判断map如果没有此车数据就发送一次订阅
//		String info =  this.gpsInfoHashMap.get(vehicleId);
//		if(info==null)sendMessage.sendToWeb(vehicleId);
		// // 压力测试 end
//		return info;
		return this.gpsInfoHashMap.get(vehicleId); //非压力测试启用
	}
	 ...
	 * @return 
	 * */
	public String getAreaInfo(String vehicleId) throws Exception {
		return this.areaInfoHashMap.get(vehicleId); 
	}
	 ...
	 * @return 此车辆的报警数据
	 * */
	public Queue<String> getAlarm() throws Exception {
		return alarmHandleThread.getQueue();
	}
	...
	public boolean findIsHaveAlarm(SessionUser user)
	{
		...
		{       
			if(UserContext.isBsRootUser()){//admin超级用户
					Queue<String> businessQueue = alarmHandleThread.getQueue();
							...
							if (businessQueue != null)
							{
								...
								for (String str : businessQueueStrs)
								{
									if(StringUtils.isNotEmpty(str)){
										String[] tempStr = str.split("=====");
										String dateStr = tempStr[1];
												if(vehicleMap.get(tempStr[2])!=null && Tools.isNew(dateStr))
												{
													/**去掉已经处理的数据【ljy】【车辆ID-报警时间-报警类型】begin**/
													String[] strs = tempStr[0].split(",");;
													String alarmStr =strs[0]+"-"+strs[1]+"-"+strs[4];
								...
								try
								{
									...
									if(StringUtils.isNotEmpty(str))
									{
										...
										if("9".equals(alarmSourceNo)){ //其它报警
											...
											if(alarmSourceNo.equals("ty")){ //【用作紧急报警{车载终端，平台报警}】
												if(!("1".equals(strs[5])) && !("10".equals(strs[5]))){
													continue;
												}
											}else{ //【实时报警的 {全部来源，车载终端，平台报警}】
										...
										if (Tools.isNew(dateStr))
										{
											...
											{// 返回50条 
												String vehicleInfo = getVehicleMessage(tempStr[2]);
							...
							if (businessQueue != null)
							{
								...
								for (String str : businessQueueStrs)
								{
									if(StringUtils.isNotEmpty(str))
									{
										...
										String alarmTime = sdf.format(date);
										String[] strs = tempStr[0].split(",");
										if(vehicleMap.get(tempStr[2])==null)continue;
										if (!("-1".equals(alarmTypeNo)) && !(alarmTypeNo.equals(strs[4])))
										{// 如果选择过滤报警
											continue;
										}
							...
							try
							{
								...
								if(StringUtils.isNotEmpty(str))
								{
									...
									if (totalCount < maxCount)
									{
										...
										String alarmTypeId1=String.valueOf(strs[4].toString().trim());
										AlarmModel alarmModel=null;
										if(list!=null){
											for (int j = 0; j < list.size(); j++) {
												String alarmTypeId = String.valueOf(list.get(j).get("alarmTypeId"));
												if(alarmTypeId.equals(alarmTypeId1)){
													alarmModel = Tools.alarmQueueTypeToList1(strs,vehicleInfo, alarmTime,alarmTypeId.toString().trim());
...
//						 String str=businessQueueStrs[alarmIndex];
						...
						if (StringUtils.isNotEmpty(str)) {
							...
							if (totalCount < maxCount) {
								...
								val.put("alarmDesc", alarmModel.getDesc());// 报警描述
								val.put("alarmTypeId", alarmModel.getAlarmTypeId());//报警类型Id
//								System.out.println("报警类型Id:---------------"+alarmModel.getAlarmTypeId());
								resultls.add(val);
	...
	public String getDriverMessage(String vehicleId){
		return commandResultHashMap.get("driver"+vehicleId);
	}
	...
	public Map<String, Object> getNewestDriverMessage(String vehilceId) {
		...
		try {
			...
			if(affixationBean!=null){
				String DrivinName=affixationBean.getDrivinName().toString();
				String DriverIC=affixationBean.getDriverIC().toString();
				String DrivingLicence=affixationBean.getDrivingLicence().toString();
				String ZdDriverCode=affixationBean.getZdDriverCode().toString();
				String ZdWhetherIC=affixationBean.getZdWhetherIC().toString();
				
				//司机姓名
				if(DrivinName!=null && DrivinName.length()>0){
					result.put("name",DrivinName);
				}
				//司机IC卡号
				if(DriverIC!=null && DriverIC.length()>0){
					result.put("driverIC",DriverIC);
				}
				//司机驾驶证号
				if(DrivingLicence!=null && DrivingLicence.length()>0){
					result.put("drivingLicence", DrivingLicence);
				}
				//终端司机编号
				if(ZdDriverCode!=null && ZdDriverCode.length()>0){
					result.put("zdDriverCode", ZdDriverCode);
				}
				//终端是否插入IC
				if(ZdWhetherIC!=null  && ZdWhetherIC.length()>0){
					result.put("zdWhetherIC",ZdWhetherIC);
				}
			}
