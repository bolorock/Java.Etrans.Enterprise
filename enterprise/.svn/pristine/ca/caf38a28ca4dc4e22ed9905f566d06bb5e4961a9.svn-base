...
$(function() {
	...
	load();
		...
		$("#speedGZSJWResultList").flexigrid( {
			url : 'analyse/driving/findSpeedGZSJWResultList.action',
			dataType : 'json',
			params : params,
			colModel : [ 
			{
...
*/
function toSearch(){
	$("#speedGZSJWResultList").hide();
	//查询参数
	var params=getparams();
	// 重置表格的某些参数
	$("#speedGZSJWResultList").flexOptions({
			newp : 1,// 设置起始页
			params : params// 设置查询参数
		}).flexReload();// 重新加载
	$("#speedGZSJWResultList").show();
}
...
function toExportExl() {
	exportExl('speedGZSJWResultList', 'analyse/driving/speedGZSJWResultExport.action');
}
