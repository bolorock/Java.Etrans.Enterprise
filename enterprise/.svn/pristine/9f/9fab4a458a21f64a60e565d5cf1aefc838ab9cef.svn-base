...
function getCommandParamSucc(data){
	if(data!=null && data.length>0){
		...
		replyFunctionNo = data[0]["replyFunctionNo"];
		if(replyFunctionNo=='0001'){
			replyFunctionNo = data[0]["functionNo"];
		}
...
function sendMessage() {
	...
	$("[id^='param']").each(function(idx1) {
      ...
      /*
			...
			*/
			message+= paramType + "@" + trim(currentValue)+"=|=";
...
function sendCommandMessage(paramMeassageValue) {
	$("#result").html("");	
	var commandName = $("#commandName").html();//二级指令名称
	specialFlag = SpecialHandle();
	var commandId = $("#commandId").val();
	var vehicleId= $.query.get('vehicleId');//车辆id
	var jsonParams = {
...
function getCommandResult() {	
	...
	if(kind==800){
		jsonParams = {
				vehicleId : replyFunctionNo+"|"+vehicleId,
...
function loadMediaMuilt(data){
	   ...
	   if(index>-1){
	    ...
	    var table='<table id="eventList"  class="form"><thead><tr><th>多媒体ID</th><th>类型</th><th>通道ID</th><th>事件</th><th>定位时间</th><th>经度</th><th>纬度</th></tr></thead>';
	    ...
	    var dataArray=newData.split("#");
		  for(var i=0;i<dataArray.length-1;i++){
			  ...
			  table+='<tr>'
				   +'<td align="left">'+resultArray[6].split(":")[1]+'</td>'
...
function SpecialHandle(){
	...
	if(commandName=="摄像头立即拍摄命令"||commandName=="存储多媒体数据上传命令"||commandName=="录音开始命令" || commandName=="单条存储多媒体数据检索上传"){
		//摄像头立即拍摄命令
		return "image";
	}else if(commandName=="存储多媒体数据检索"){
		return "mediaMuilt"
	}
	else{//不用做特殊处理
		return "-1";
