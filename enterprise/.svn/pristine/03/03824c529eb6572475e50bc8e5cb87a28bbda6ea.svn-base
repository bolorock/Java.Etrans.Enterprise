...
import com.etrans.bubiao.services.query.stat.securityManager.VehicleOSTimesStatService;
...
public class VehicleOSTimesStatAction extends BaseAction {  
	
	
	
	public VehicleOSTimesStatAction() {
//		super();
		super.excelTplFile="超速行驶管理";//导出文件名称
	}
	...
	@Autowired
	private VehicleOSTimesStatService  vehicleOSTimesStatService;
	
	/**
	 * 查询数据列表
	 */
	@Action(value = "findvehicleOSTimesStatList")
	public void findvehicleOSTimesStatList(){
		try {
			...
			log.error(DateUtil.getCurrentTime("yyyy-M-dd HH:mm:ss")+"-----------安全管理--》【超速行驶】Action--》【查询数据列表】【开始】----------------");
			this.renderJSON(JSONUtil.toJson(vehicleOSTimesStatService.getVehicleOSTimesStatList(paramMap)));
			LogUtil.insertLog(LogActionTypes.READ, "成功", "超速行驶管理", "", "查询超速行驶管理");
		} catch (Exception e) {
			LogUtil.insertLog(LogActionTypes.READ, "失败", "超速行驶管理", "", "查询超速行驶管理");
			log.error("安全管理--》超速行驶Action--》查询数据列表报错！报错信息如下：");
			e.printStackTrace();
		}finally{
			log.error("-----------安全管理--》超速行驶Action--》【查询数据列表】【结束】----------------");
        }
    ...
    protected void doFillWorkbook(Workbook wb) {
		...
		//查询导出数据列表
		List<Map<String, Object>> vehicleModels = vehicleOSTimesStatService.getVehicleOSTimesStatListExportExl(paramMap, fromPage, toPage);
		
		//列名数组，对应对象的字段
		String [] titleArray={"RegistrationNo","WeekOSTimes","MonthOSTimes","YearOSTimes","WeekOSTimes_oWeek","WeekNumber","DayOSTimes_oMonth",
		...
		List<Object> list = new ArrayList<Object>();
    		...
    		log.error(DateUtil.getDatePattern()+"安全管理--》疲劳驾驶Action--》导出数据报错！报错信息如下："+e.getMessage());
			e.printStackTrace();
		}
    }
     ...
     * @return
     */
    @Action(value = "doVehicleOSTimesStatListExportExl", results = @Result(type = "stream", params = { "contentType", "application/vnd.ms-excel",
            "bufferSize", "1024", "contentDisposition", "attachment;filename=${excelTplFile}.xls" }))
	public String doVehicleOSTimesStatListExportExl(){
    	try {
    			log.error(DateUtil.getCurrentTime("yyyy-M-dd HH:mm:ss")+"-----------安全管理--》【超速行驶】Action--》【导出】【开始】----------------");
	            ...
	            log.error("安全管理--》超速行驶Action--》导出数据报错！报错信息如下："+e.getMessage());
	        } finally{
	        	log.error("-----------安全管理--》超速行驶Action--》【导出】【结束】----------------");
	        }
	        return SUCCESS;
    }
