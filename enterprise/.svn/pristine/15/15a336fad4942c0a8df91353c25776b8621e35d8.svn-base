$(function() {
		$("#recordList").flexigrid( {
			...
			colModel : [ 
			{
					...
					display : '最大停车时间',
					name : 'bounceParkTimeDay',
					width : 107,
					sortable : true,
					align : 'left'
				},{
					display : '点范围门限',
					name : 'bounceParkDistance',
					width : 107,
					sortable : true,
					align : 'left'
				},{
...
//表名
var tableName = "ANA_AnalyserOverTimePark";
//编辑权限
var editAction = "editOverTimePark";
//删除权限
var delAction = "deleteOverTimePark";

var DXMTypeName="placeTypeId";
/**
 * 查询参数
 */
function getParam(){
...
function getAddForm(){
	...
	var isPlaceInAlarm = $("#isPlaceInAlarm").val();//分析方式
	var placeTypeId = $("#placeTypeId").val();//地点组
	var bounceParkTimeDay = $("#bounceParkTimeDay").val();//最大停车时间
	var bounceParkDistance = $("#bounceParkDistance").val();//点范围门限
	var checkInterval = $("#checkInterval").val();//检测间隔(秒)
	...
	var params = {
			...
			isPlaceInAlarm : isPlaceInAlarm,
			placeTypeId : placeTypeId,
			bounceParkTimeDay : bounceParkTimeDay,
			bounceParkDistance : bounceParkDistance,
...
function updateAddForm(){
	...
	var checkInterval = $("#checkInterval").val();//检测间隔(秒)
	 ...
	 var setParam = new QueryParam();
		 ...
		 setParam.put("@isPlaceInAlarm",isPlaceInAlarm);
		 //setParam.put("@placeTypeId",placeTypeId);
		 setParam.put("@bounceParkTimeDay",bounceParkTimeDay);
		 setParam.put("@bounceParkDistance",bounceParkDistance);
		 setParam.put("@checkInterval",checkInterval);
...
function load(){
	...
	/**其它报警设置相同业务【指有时间类型业务更这个相同的】必须复制begin**/
		...
		//初始化分析方式选择框
		initPlaceInAlarm("isPlaceInAlarm");
//		//初始化地点组选择框
//		initAjaxSelect("placeTypeId","sys/getPlaceTypeId.action",0);
		
		//初始化地点选择框
		initAjaxSelect_Ansynce("placeTypeId","sys/getPoints.action",0,false);
...
function doCreate(){
	...
	if(canSubmit){
		...
		$.ajax({
		    type : "POST",
		    url : "analyse/insertAnalyseOverTimePark.action",
