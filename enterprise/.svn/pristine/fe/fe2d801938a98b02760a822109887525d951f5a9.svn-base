...
import com.etrans.bubiao.services.query.stat.OilServices;
...
public class OilAction extends BaseAction {
	...
	public OilAction() {
		super.excelTplFile = "OilInfoModel";
	}
	 ...
	 * 
	 */
	@Action(value = "findOilInfo")
	public void findOilInfo() {
		try {
			log.error(DateUtil.getCurrentTime("yyyy-M-dd HH:mm:ss")+"-----------安全管理--》【油量统计】Action--》【查询数据列表】【开始】----------------");
			SessionUser user = UserContext.getLoginUser();
			this.renderJSON(JSONUtil.toJson(oilServices.getOilInfo(this
					.listParam(), user)));
			LogUtil.insertLog(LogActionTypes.READ, "成功", "车辆油量统计", "", "查询车辆油量统计");
		} catch (Exception e) {
			LogUtil.insertLog(LogActionTypes.READ, "失败", "车辆油量统计", "", "查询车辆里程统计");
			log.error("安全管理--》油量统计Action--》查询数据列表报错！报错信息如下："+e.getMessage());
			e.printStackTrace();
		}finally{
			log.error("-----------安全管理--》油量统计Action--》【查询数据列表】【结束】----------------");
		}
	...
	protected void doFillWorkbook(Workbook wb) {
		try {
			...
			SessionUser user = UserContext.getLoginUser();
			List<Map<String, Object>> vehicleModels = oilServices
					.getOilInfoExl(this.listParam(), user, fromPage, toPage);
			String[] titleArray = { "registrationNo", "WeekOil", "MonthOil",
			...
			ExcelUtil.writeHashMapToExcel(wb.getSheetAt(0), list, titleArray,
					4, 0);
			LogUtil.insertLog(LogActionTypes.READ, "成功", "车辆油量统计", "", "导出车辆油量统计");

		} catch (Exception e) {
			LogUtil.insertLog(LogActionTypes.READ, "失败", "车辆油量统计", "", "导出车辆油量统计");
			log.error(DateUtil.getDatePattern()+"安全管理--》油量统计Action--》导出数据报错！报错信息如下："+e.getMessage());
			e.printStackTrace();
		}
	}
	 ...
	 * 
	 */
	@Action(value = "OilExportExl", results = @Result(type = "stream", params = {
	...
	public String OilExportExl() throws UnsupportedEncodingException {
		...
		try {
			log.error(DateUtil.getCurrentTime("yyyy-M-dd HH:mm:ss")+"-----------安全管理--》【油量统计】Action--》【导出】【开始】----------------");
			this.excelTplFile = new String(this.excelTplFile.getBytes(),
					"ISO-8859-1");
		} catch (UnsupportedEncodingException e) {
			log.error("安全管理--》油量统计Action--》导出数据报错！报错信息如下："+e.getMessage());
		}finally{
			log.error("-----------安全管理--》油量统计Action--》【导出】【结束】----------------");
        }
		return SUCCESS;
	}
	...
	private Map<String, Object> listParam() throws UnsupportedEncodingException {
		...
		if (getParameter("flag").toString().equals("1")) {
			registrationNo = new String(getParameter("registrationNo")
