...
var playSpeed=100;
var showStatusOld=null;
var showStopOld=null;
var showSpeedOld=null;
var showLocationOld=null;
...
function findPlayBackTrack() {
	...
	if(vehicleOld==null){
		...
		showStatusOld=$("#chkStatus").is(":checked");
		showStopOld=$("#chkStop").is(":checked");
		showSpeedOld=$("#chkSpeed").is(":checked");
		showLocationOld=$("#chkLocation").is(":checked");
	...
	if (isMustQuery) {
		...
		var countQuery = 0;
		var showStatus=$("#chkStatus").is(":checked") ? 1:0;	//是否显示状态
		var isMergeStop=$("#chkStop").is(":checked") ? 1:0;		//是否合并停车轨迹点
		var isMergeSpeed=$("#chkSpeed").is(":checked") ? 1:0;	//是否合并速度
		...
		for ( var bt = beginTime; bt <endTime; bt = (bt + getDataTime)) {
			...
			var jsonParams = {
				...
				isMergeStop:isMergeStop,
				isMergeSpeed:isMergeSpeed,
				index:countQuery
			};
			countQuery++;
			// ljy地图类型
			...
			$.ajax({
				...
				success : function(data) {
					...
					if (finshTime >= endTime && data == 'none') {// 获取轨迹数据结束
...
function createTrackTable(){
	if(aynchronizedLngLatMap.size()>0){
		...
		$('#mytable').html("");
		var strHtml="";
		var showStatus=$("#chkStatus").is(":checked");			//是否显示状态
		var isShowLocation=$("#chkLocation").is(":checked");		//显示位置
		
		if (showStatus){
			var $tbTrack=$("#tbTrack tr");
			$tbTrack.append("<td class=\"m_t\" nowrap=\"nowrap\" width=\"15%\">状态</td>");
		}
		var num=0;
		for(var i=0;i<mapKeys.length;i++){  
			...
			if(gpsInfoArray!=null){
				...
				for ( var j = 1; j < arrLen; j++) {
					...
					idStr = xy.sim+i+j;
					num+=1;
					strHtml+="<tr><td style='border:0px solid;' align='center' width='5%' data-value='"+num+"'>"+num+"</td>";// 编号
					strHtml+="<td style='border:0px solid;' align='center' width='9%' data-value='"+xy.no+"'>"+xy.no+"</td>";// 车牌
					...
					strHtml+="<td style='border:0px solid;' align='center' width='18%' data-value='"+xy.gt+"'>"+xy.gt+"</td>";// 时间
...
function showLocation(){
	...
	for(var i=0;i<len;i++){
		...
		var lon=$td.eq(3).html();
		var $div=$td.eq(9).find("div");
...
function lineDraw(gpsInfoArrayTemp) {
	...
	for ( var i = 1; i < arrLen; i++) {
		...
		if (speed<=10 && isStopPos && !isEnd){//速度小于10，视为停车
			continue;
		}
		...
		if (speed<=10){//速度小于10，视为停车
			...
			if (isStopPos){
				...
				createStopLabel(stopGpsInfo,stopPos,beginTime,endTime);
			}
			isStopPos=false;
		}
...
function crateStopIcon(xy,stopPos,beginTime,endTime){
	markerUtils_B.setParamValue(15,23,"imgs/car/point2.gif",true);
...
function createStopLabel(xy,stopPos,beginTime,endTime){
	var opts = {
			  position : stopPos ,   // 指定文本标注所在的地理位置
			  offset   : new BMap.Size(-8,-27)    //设置文本偏移量
			};
	var text="停车时间："+toTime(timeDiff(beginTime,endTime));
...
function exportTrack(basePath){
		...
		var $titles=$("<input type='hidden' name='titles' />");
		var titles=getTitles();
		var data=getTableData();
        $input.attr('value',data);
        $titles.attr('value',titles);
        $("#ddform").append($input);
        $("#ddform").append($titles);
...
function showImageForPoint(sHlon,sHlat) {
	newMarker_point =markerUtils_B.crateIconMarker(new BMap.Point(sHlon, sHlat), 1);
	maplet.addOverlay(newMarker_point);
	maplet.panTo(new BMap.Point(sHlon, sHlat));
}
...
$(function(){
	$("#mytable tr").live({
	    mouseenter: function () {
	    	var $td=$(this).find("td");
			var lon=Number($td.eq(2).html())+0.003489415;
			var lat=Number($td.eq(3).html())+0.010439549;
			showImageForPoint(lat,lon);
	    },
	    mouseleave: function () {
			maplet.removeOverlay(newMarker_point);
	    }
	});
});
