...
/**
 * 安全管理--》超速行驶Service(按部门)
 * @author lujunyong
 *
 */
@Service
public class WorkUnitOSTimesStatService {
	...
	public PageBean getWorkUnitOSTimesStatList(Map setParamsMap) throws Exception{
		...
		try {
			...
			/**执行查询**/
    		rows= this.ibatisServices.queryForList(Map.class, "getWorkUnitOSTimesStatSQL",setParamsMap);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public  List<Map<String, Object>>  getWorkUnitOSTimesStatListExportExl(Map setParamsMap,String fromPage,String toPage) throws Exception{
...
//	    setParamsMap.put("@workunitid",String.valueOf(sessionUser.getWorkUnitID()));
		...
		/**查询结果**/
		List<Map<String,Object>> rows = this.ibatisServices.queryForList(Map.
				class, "getWorkUnitOSTimesStatSQL",setParamsMap);
		return rows;
	...
	public void doLoginFile(Map setParamsMap,String type){
		...
		log.error("-----------安全管理--》超速行驶Service--》查询数据列表【传到存储过程所需参数Begin】"+type+"----------------");
		...
		log.error("-----------安全管理--》超速行驶Service--》查询数据列表【传到存储过程所需参数End】"+type+"----------------");
