...
$(function() {
	    ...
	    //加载列表
		$("#findWorkUnitOSTimesStatList").datagrid( {
			url : 'query/stat/findWorkUnitOSTimesStatList.action',
...
//	// 重置表格的某些参数
//	$("#findVehicleOSTimesStatList").datagrid({
//			newp : 1,// 设置起始页
...
//	
	...
	//载入并显示第一页的记录
	$('#findWorkUnitOSTimesStatList').datagrid('load',params);
...
function setPageControl(){
	...
	/**分页组件datagrid**/
	var p = $('#findWorkUnitOSTimesStatList').datagrid('getPager');  
    $(p).pagination({  
...
function toExportExl(){
	var options= $('#findWorkUnitOSTimesStatList').datagrid('options');
...
//	var week=34;
	...
	//alert("《超速管理》"+"企业名称："+workUnitName+"year:"+year+"month:"+month+"day:"+day+"week:"+week+"registrationNo:"+registrationNo+"page:"+page+"rows:"+rows);
	exportExlEasyui('findWorkUnitOSTimesStatList', 'query/stat/doWorkUnitOSTimesStatListExportExl.action?workUnitName='+workUnitName+'&year='+year+
			'&month='+month+'&day='+day+'&week='+week+'&page='+page+'&rows='+rows);
