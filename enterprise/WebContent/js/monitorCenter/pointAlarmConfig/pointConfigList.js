...
$(function() {
	    ...
	    $("#pointConfigList").flexigrid( {
	    	url : 'monitorCenter/findPointConfigList.action',
			dataType : 'json',
			params : params,
			colModel : [ 
			{
...
function getHandleColumn(id){
	...
	var deleteStr = "";
	if(resources!=null){
		 if(resources.indexOf("|point_view|")!=-1){
			  editStr = '<a href="javascript:void(0)" onclick="doView(' + id + ')">查看</a>';
		}
		 if(resources.indexOf("|point_delete|")!=-1){
...
function toCreate(){
	...
	//加载关键点列表
	initAjaxSelect("pointIdSel","sys/getPoints.action",0);
	//加载检测时间类型列表
...
function ConfigAdd(){
	...
	if(flag){
		...
		var pointConfig = getParams();
		var idStr = vehicleStr;
		$.ajax({
		    type : "POST",
		    //createLineOffsetConfig
		    url : "monitorCenter/createPointConfig.action",
		    data : {pointConfig:pointConfig,idStr:idStr},
		    dataType : "JSON",
		    success : function(data) {
		    	if(data.code == '0'){
		    		 hide();
		    		$("#pointConfigList").flexReload();
		    	}else{
		    		showError();
		    	}
		    },
...
function getParams(){
	...
	var pointNameTxt = $("#pointNameTxt").val();//关键点名称
	var range = $("#range").val();//范围
	var pointIdSel = $("#pointIdSel").val();//线路id
	...
	//参数
	var pointConfig = "{'pointNameTxt':'" + pointNameTxt +
	"','range':'" + range + 
	"','pointIdSel':" + pointIdSel + 
	...
	if (ids != null || ids.length > 0) {
		if (!confirm("确定删除关键点?")) {
			return false;
				...
				$.ajax({
				    type : "POST",
				    url : "monitorCenter/updatePointConfig.action",
...
function doView(id){
	...
	if(id != null && id != ''){
		...
		$.post("monitorCenter/getPointConfigById.action", {id : id}, function(data){
			if(data.code == '1'){
				...
				//相关区域点
				var pointList = config.pointPoints;
				var areaStr = "";
				areaStr += "<ul>";
				if(pointList && pointList.length>0){
					for ( var i = 0; i < pointList.length ; i++) {
						areaStr += "<li>" + pointList[i].Longitude + "," + pointList[i].Latitude + "</li>";
					}
				}
				areaStr += "</ul>";
...
function isDigit(){ 
	var test_value=document.getElementById("range").value;
	var patrn=/^([1-9]\d*|0)(\.\d*[1-9])?$/; 
	if (!patrn.exec(test_value)){
		$("#rangespan").showMessage({
			type : "error",
			closeable : false, 
			text : "请输入有效正整数！"});
		}else{
			 $("#rangespan").closeMessage();
		}
	} 
