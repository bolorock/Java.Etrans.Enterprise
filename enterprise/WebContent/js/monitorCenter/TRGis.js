...
var showLng;
var showLat;
var tempGpsInfoAry = new Array(20);
function load() {
 ...
 * @param obCar
 */
function addTempAry(obCar){
	if(tempGpsInfoAry.length>20){
		tempGpsInfoAry.pop();
	}
	tempGpsInfoAry.push(obCar);
}
function MakerCar(obCar, tableFlag) {
	//addTempAry(obCar);//过检专用,正式屏蔽
	car = new MyObject(obCar.vehicleId);		
	if (!carHashMap.containsKey(car)) {
...
//			}
			...
			var option=newMarker.getOptions();
			if (option.zindex==510){
				newMarker.closeInfoWindow();//关闭弹出窗口
				/**设置弹出框begin**/
				var infowindow = createInfoWindow(obCar);
				var infowin = newMarker.openInfoWinHtml(infowindow);
				newMarker.moveToShow();
				/**设置快捷标题**/
				infowin.setTitle(getTitleDiv(obCar.no,obCar.vehicleId,1));
			}
		} catch (e) {
...
//	var newLine="<tr id='gps"+obCar.vehicleId+"' ondblclick='reloadMapCenter("+obCar.vehicleId+");'><td><div id='cp" + obCar.vehicleId + "'><a href='monitorCenter/attention.jsp?vehicleId=" + obCar.vehicleId + "' style='cursor:hand;font-size:9pt;' target='_blank'>" + obCar.no + "</a></div></td>"
    ...
    +"<td width='19%' style='border:0px solid;' align='center'><div id='fjInfo" + obCar.vehicleId + "' title='"+(!obCar.fjInfo?'':obCar.fjInfo) +"'>"  + (!obCar.fjInfo?'':obCar.fjInfo.substr(0,10)+"……") + "</div></td>"
...
function createInfoWindow(obCar) {
	var div = '<div><table width="385" border="0" cellspacing="2" cellpadding="0"><tr bgcolor="#e6eaea">';
	div += '<td colspan="2" height="20" id="info_gs">' + obCar.gs + '</td></tr>' 
    	+ '<tr><td colspan="2" height="20" id="info_alarmState">报警状态：'+obCar.alarmState+'</td></tr>'
    	+ '<tr bgcolor="#e6eaea"><td height="18" id="info_gd">高度：' + obCar.gd + 'm</td>' + '<td id="info_sim">SIM卡：' +obCar.sim + '</td></tr>'
    	+ '<tr><td height="18" id="info_sd">速度：' + obCar.sd + 'km/h</td>' + '<td id="info_hd">方向：' + getHeadDes(obCar.hd) + '</td></tr>'
    	+ '<tr bgcolor="#e6eaea"><td height="18" id="info_lon">经度：' + obCar.lon+ '</td>' + '<td id="info_lat">纬度：' + obCar.lat + '</td></tr>' 
	   + '<tr><td height="18" id="info_kindName">所属行业：' + obCar.kindName+ '</td>' + '<td id="info_workunitName" >所属业户：' + obCar.workunitName + '</td></tr>' 
	   + '<tr bgcolor="#e6eaea"><td height="18" colspan="2" id="info_bill">电子运单：' + obCar.bill+ '</td></tr>' 
	    +'<tr><td><a href="javascript:void(0)" onclick=getDriverMessage("'+obCar.vehicleId+ '")> 查看司机信息</a></td><td id="info_location"><a href="javascript:void(0)" onclick=getLocationInfo2("' + obCar.vehicleId + '","' + obCar.lon + '","' + obCar.lat + '")>显示位置</a></td></tr>'
	   +'<tr bgcolor="#e6eaea"><td colspan="2"  height="50" id="message">&nbsp;</td></tr></table></div>';
			
	div = div
			+ "<table width='100%' height='12px' cellspacing='0' cellpadding='0'><tr><td><span style='color:#0000cc;cursor:pointer;text-decoration:underline;font-size:12px;' onclick='javascript:map.closeInfoWindow();'></span></td>";
	div = div + "<td style='font-size:12px;' align='right' id='info_gt'>定位时间:" + obCar.gt + "</td></tr></table>";
	return div;
}
function updateInfoWindow(obCar){
	$("#info_gs").html(obCar.gs);
	$("#info_alarmState").html("报警状态："+obCar.alarmState);
	$("#info_gd").html("高度："+obCar.gd+"m");
	$("#info_sim").html("SIM卡："+obCar.sim);
	$("#info_sd").html("速度："+obCar.sd+"km/h");
	$("#info_hd").html("方向："+getHeadDes(obCar.hd));
	$("#info_lon").html("经度："+obCar.lon);
	$("#info_lat").html("纬度："+obCar.lat);
	$("#info_kindName").html("所属行业："+obCar.kindName);
	$("#info_workunitName").html("所属业户："+obCar.workunitName);
	$("#info_bill").html("电子运单："+obCar.bill);
	$("#info_location").html('<a href="javascript:void(0)" onclick=getLocationInfo2("' + obCar.vehicleId + '","' + obCar.lon + '","' + obCar.lat + '")>显示位置</a>');
	$("#info_gt").html("定位时间:"+obCar.gt);
}
// 放大
...
//车辆id
//var vehicleIdString;
/***关闭单车跟踪的弹出信息框【和百度地图同步】***/
function closTitleWin(){
//	if(vehicleIdString){//不为空
//		alert("主监控关闭弹出框：车辆id"+vehicleIdString);
//		car = new MyObject(vehicleIdString);
//		var newMarker = carHashMap.get(car);//图片标注对象
//		newMarker.closeInfoWindow();//关闭弹出窗口
//	}
}
