...
$(function() {
	...
	load();
		...
		$("#drivingOverSpeedRoadList").flexigrid( {
			url : 'analyse/driving/findDrivingOverSpeedRoadList.action',
			dataType : 'json',
			params : params,
			colModel : [ 
			{
...
*/
function toSearch(){
	$("#drivingOverSpeedRoadList").hide();
	//查询参数
	var params=getparams();
	// 重置表格的某些参数
	$("#drivingOverSpeedRoadList").flexOptions({
			newp : 1,// 设置起始页
			params : params// 设置查询参数
		}).flexReload();// 重新加载
	$("#drivingOverSpeedRoadList").show();
}
...
function toExportExl() {
	exportExl('drivingOverSpeedRoadList', 'analyse/driving/drivingOverSpeedRoadExport.action');
}
