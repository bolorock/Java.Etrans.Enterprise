...
$(function() {
	...
	load();
		...
		$("#drivingDriverOverSpeedList").flexigrid( {
			url : 'analyse/driving/findDrivingDriverOverSpeedList.action',
			dataType : 'json',
			params : params,
			colModel : [ 
			{
...
*/
function toSearch(){
	$("#drivingDriverOverSpeedList").hide();
	//查询参数
	var params=getparams();
	// 重置表格的某些参数
	$("#drivingDriverOverSpeedList").flexOptions({
			newp : 1,// 设置起始页
			params : params// 设置查询参数
		}).flexReload();// 重新加载
	$("#drivingDriverOverSpeedList").show();
}
...
function toExportExl() {
	exportExl('drivingDriverOverSpeedList', 'analyse/driving/DrivingDriverOverSpeeExport.action');
}
