...
$(function() {
		...
		$("#analyseMileagePerHourList").flexigrid( {
			url : 'analyse/driving/findAnalyseMileagePerHourList.action',
			dataType : 'json',
			params : params,
			colModel : [ 
			{
...
*/
function toSearch(){
	$("#analyseMileagePerHourList").hide();
	//查询参数
	var params=getparams();
	// 重置表格的某些参数
	$("#analyseMileagePerHourList").flexOptions({
			newp : 1,// 设置起始页
			params : params// 设置查询参数
		}).flexReload();// 重新加载
	$("#analyseMileagePerHourList").show();
}
...
function toExportExl() {
	exportExl('analyseMileagePerHourList', 'analyse/driving/analyseMileagePerHourExport.action');
}
