...
$(function() {
	...
	load();
		...
		$("#drivingOverSpeedList").flexigrid( {
			url : 'analyse/driving/findDrivingOverSpeedList.action',
			dataType : 'json',
			params : params,
			colModel : [ 
			{
				...
				display : '超速开始时间',
				name : 'OverSpeedTimebegin',
				width :150,
				sortable : true,
				align : 'left'
			}, {
				display : '超速截止时间',
				name : 'OverSpeedTimeEnd',
				width :150,
				sortable : true,
				align : 'left'
			},{
...
*/
function toSearch(){
	$("#drivingOverSpeedList").hide();
	//查询参数
	var params=getparams();
	// 重置表格的某些参数
	$("#drivingOverSpeedList").flexOptions({
			newp : 1,// 设置起始页
			params : params// 设置查询参数
		}).flexReload();// 重新加载
	$("#drivingOverSpeedList").show();
}
...
function toExportExl() {
	exportExl('drivingOverSpeedList', 'analyse/driving/drivingOverSpeedExport.action');
}
