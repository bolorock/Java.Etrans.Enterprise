		...
		$("#recordList").flexigrid( {
			...
			colModel : [ 
			{
				...
				,{
					...
					display : '疲劳驾驶临界时间(秒)',
					name : 'DrivingOverTime',
					width : 107,
					sortable : true,
					align : 'left'
				},{
					display : '最小判断距离(公里)',
					name : 'DrivingMinMileage',
					width : 107,
					sortable : true,
					align : 'left'
				},{
					display : '最小有效休息时间(秒)',
					name : 'MinValidRestTime',
					width : 107,
					sortable : true,
					align : 'left'
				},{
...
//表名
var tableName = "ANA_AnalyserDOTGZSJW";
//编辑权限
var editAction = "editDOTGZSJW";
//删除权限
var delAction = "deleteDOTGZSJW";
...
function getAddForm(){
	...
	var alertInterval = $("#alertInterval").val();//报警间隔
	var isRecordResult = $("#isRecordResult").val();//是否记录
	
	var earlyWarningLimit = $("#earlyWarningLimit").val();//预警门限(秒)
	var continuousRestTime = $("#continuousRestTime").val();//连续休息时间(秒)
	var sumOfRestTime = $("#sumOfRestTime").val();//累计休息时间(秒)
	var drivingOverTime = $("#drivingOverTime").val();//疲劳驾驶临界时间(秒)
	var drivingMinMileage = $("#drivingMinMileage").val();//最小判断距离(公里)
	var minValidRestTime = $("#minValidRestTime").val();//最小有效休息时间(秒)
	 ...
	 setParam.put("@alertInterval",alertInterval);
	 setParam.put("@isRecordResult",isRecordResult);
	 
	 setParam.put("@earlyWarningLimit",earlyWarningLimit);
	 setParam.put("@continuousRestTime",continuousRestTime);
	 setParam.put("@sumOfRestTime",sumOfRestTime);
	 setParam.put("@drivingOverTime",drivingOverTime);
	 setParam.put("@drivingMinMileage",drivingMinMileage);
	 setParam.put("@minValidRestTime",minValidRestTime);
...
function load(){
	...
	/**其它报警设置相同业务【指有时间类型业务更这个相同的】必须复制begin**/
		...
		initIsWarn("isWarn");
		//初始是否记录结果选择框
		initIsRecordResult("isRecordResult");
