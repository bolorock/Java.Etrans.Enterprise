...
function initGrid(){
		$("#recordList").flexigrid( {
			url : 'analyse/findAnalyserOverSpeedRoadCYList.action',
			dataType : 'json',
...
function doEdit(id){
		if(id != null && id != ''){
			...
			var url="analyse/getAnalyserOverSpeedRoadCYById.action";
		...
		if(recordInfo.length > 0){
			...
			$("#editWindow select").each(function(i){
				...
				if($(this).attr("id")=="placeTypeID"){
					//alert("特殊下拉框:"+recordInfo[0]["polygonName"]);
					initSelectDXM("placeTypeID",recordInfo[0]["polygonName"]);
				}
			});
...
function doUpdate(){
	...
	if(canSubmit){
		...
		$.ajax({
		    type : "POST",
		    url : "analyse/updateOverSpeedRoadCY.action",
		    data : params,
		    dataType : "JSON",
...
function doCreatedo() {
	...
	if(flag){
		var url = "analyse/createOverSpeedRoadCY.action";
		var params = getAddForm();
...
function load(){
	...
	/**其它报警设置相同业务【指有时间类型业务更这个相同的】必须复制begin**/
		...
		//初始化区域名称
		initAjaxSelect_Ansynce("placeTypeID","sys/getCustomArea.action",0);
...
function getAddForm() {
	...
	var alertInterval = $("#alertInterval").val();//报警间隔(秒) 
	...
	var placeTypeID = $("#placeTypeID").val();//区域
	var id = $("#id").val();//id
...
function doDelete(ids){
	...
	if (ids != null || ids.length > 0) {
		if (!confirm("确定删除选中的信息?")) {
			...
			$.ajax({
			    type : "POST",
			    url : "analyse/deleteOverSpeedRoadCY.action",
