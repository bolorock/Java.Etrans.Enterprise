...
//表名
var tableName = "ANA_AnalyserRoadSpeed";
//编辑权限
var editAction = "editAnalyseRoadSpeed";
//删除权限
var delAction = "deleteAnalyseRoadSpeed";

//点线区域下拉框id
var DXMTypeName ="roadSegmentID";
...
function getAddForm(){
	...
	var roadSegmentID = $("#roadSegmentID").val();//检测路段
	var isMoreThan = $("#isMoreThan").val();//检测方式
	var bounceSpeed = $("#bounceSpeed").val();//速度
...
//	 
//	 setParam.put("@roadSegcheckTimeBeginmentID",roadSegmentID);
...
function updateAddForm(){
	...
	var bounceSpeed = $("#bounceSpeed").val();//速度
	 ...
	 setParam.put("@roadSegmentID",roadSegmentID);
	 setParam.put("@isMoreThan",isMoreThan);
	 setParam.put("@bounceSpeed",bounceSpeed);
...
//	controlYesOrNo("reSetBtn","yes");
	//检测路段下拉框可用
	controlYesOrNo("roadSegmentID","yes");
...
function doCreate(){
	...
	if(canSubmit){
		...
		$.ajax({
		    type : "POST",
		    url : "analyse/insertAnalyseRoadSpeed.action",
...
function load(){
	...
	/**其它报警设置相同业务【指有时间类型业务更这个相同的】必须复制begin**/
		...
		//初始化检测路段选择框
//		initAjaxSelect("roadSegmentID","sys/getRoadSegmentID.action",0);
		initAjaxSelect_Ansynce("roadSegmentID","sys/initLinKinds.action",0,false);
...
//	controlYesOrNo("reSetBtn","no");
	...
	doEdit(id);
	//检测路段下拉框不可用
	controlYesOrNo("roadSegmentID","no");
}
