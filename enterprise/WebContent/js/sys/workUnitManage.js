...
function initTree() {
	...
	$('#workUnitMenulist').tree({
		...
		onClick : function(node) {
			...
			$("#workUnitName").val("");
			var params = [{name : 'parentFullId',value : parentNodeId.replace(/[ ]/g,"")},{name: 'level',value : "subordinate"}];
			gridReload(params);
		}
	});
...
function showArea(params) {
	...
	$("#workUnitList").flexigrid( {
		...
		singleSelect : false,
		width : getWidth()-500,// 表格宽度
		height : getNormalHeight()-28//表格高度
			});
}
 ...
 * @returns {String}
 */
function getHandleColumn(id,name){
	var editStr = "";
	var deleteStr = "";
	var addSonUnit="";
	...
	if(resources!=null){
		//判断ACTION的访问权限
		 ...
		 if(resources.indexOf("|workUnitManage_delete|")!=-1){
			 deleteStr = '<a href="javascript:void(0)" onclick="doDelete(\''+id+'\',\''+name+'\')">删除</a>';
		 }
