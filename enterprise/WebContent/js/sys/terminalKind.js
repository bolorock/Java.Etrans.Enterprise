...
function initGrid(){
		$("#terminalKindList").flexigrid( {
			url : 'sys/terminalKindList.action',
			dataType : 'json',
			colModel : [
             {
 				display : '终端类型名称',
...
function getHandleColumn(id){
	...
	if(resources!=null){
		//判断ACTION的访问权限
		 if(resources.indexOf("|updateTerminalKind|")!=-1){
			 editStr ='<a href="javascript:void(0)" onclick="doEdit(' + id + ')">编辑</a>'
		 }
		 if(resources.indexOf("|deleteTerminalKind|")!=-1){
...
function toSearch(){
	...
	var name = $("#namePram").val();
	var params = [{
		name : 'name',
		value : name
	}];
	// 重置表格的某些参数
	$("#terminalKindList").flexOptions({
			newp : 1,// 设置起始页
...
function doCreate(){
	...
	if(canSubmit){
		...
		$.ajax({
		    type : "POST",
		    url : "sys/createTerminalKind.action",
		    data : {params : $.toJSON(params)},
		    dataType : "JSON",
		    success : function(data) {
		    	if(data != null){
		    		hide();
		    		$("#terminalKindList").flexReload();
		    	}else{
		    		showError();
		    	}
		    },
 ...
 */
function toEdit(){
	var checkedIds = $("#terminalKindList").getCheckedRows();
...
function doEdit(id){
	...
	if(id != null && id != ''){
		...
		$.ajax({
		    type : "POST",
		    url : "sys/getTerminalKindById.action",
		    data : {id:id},
		    dataType : "JSON",
		    success : function(data) {
		    	if(data!= null){
...
function doUpdate(){
	...
	if(canSubmit){
		...
		$.ajax({
		    type : "POST",
		    url : "sys/updateTerminalKind.action",
		    data : {params : $.toJSON(params)},
		    dataType : "JSON",
		    success : function(data) {
		    	if(data!= null){
...
function doDelete(ids){
	...
	if (ids != null || ids.length > 0) {
		if (!confirm("确定删除选中的终端类型信息?")) {
			return false;
		} else {
			$.ajax({
			    type : "POST",
			    url : "sys/deleteTerminalKind.action",
