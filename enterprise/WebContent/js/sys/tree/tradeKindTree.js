	...
	$.ajax({
		...
		success:function(result){
			if(result!=null && result.length>0){
			    ...
			    var str="<tr><td>&nbsp;行业类型</td></tr>";
			
			    for(var i=0;i<objs.length;i++){
		    		str+="<tr><td>&nbsp;<input type='checkbox'  name='tradeKindId' id='tradeKindId' onclick='onClickItem()' "+vaildCheck(objs[i])
...
function onClickItem(){
	var obj = $(":checkbox[name='tradeKindId']");
	var selObj = $(":checkbox[name='tradeKindId']:checked");
...
function functionAssign() {
	...
	if(ids !=''){
		if(selObj.length>1){
			...
			parWin.document.getElementById("tradeKindId").value =strs[0];
	    	parWin.document.getElementById("typeName").value =strs[1];
			window.close();
		}
	}else{
		alert('请选择行业类型！');
