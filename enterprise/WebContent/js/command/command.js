...
$(function() {
	...
	terminalKindID=$.query.get('terminalKindID');//mainFrame[0]; 终端类型
	vehicleId=$.query.get("vehicleId");//mainFrame[1]; 车辆id
	kind=$.query.get("kind");//mainFrame[2]; 车辆id
	//按钮绑定点击事件
    getFunctionCode();
...
function getPubCommandCode(){
	...
	var commandKindID = selectCommand.attr("commandKindID"); //一级指令id
	var commandId=selectCommand.attr("commandId"); //二级指令id
	var src="command/gbIndex.jsp?commandKindID="+commandKindID+"&commandId="+commandId+"&vehicleId="+vehicleId+"&kind="+kind;
