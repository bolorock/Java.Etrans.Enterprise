...
$(function() {
		...
		$("#vehicleOnlinePercertList").flexigrid( {
			url : 'query/stat/findVehicleOnlinePercertList.action',
			dataType : 'json',
			params : params,
			colModel : [ 
			  {
...
*/
function toSearch(){
	$("#vehicleOnlinePercertListChart").hide();
	//查询参数
	var params=getparams();
	// 重置表格的某些参数
	$("#vehicleOnlinePercertList").flexOptions({
			newp : 1,// 设置起始页
			params : params// 设置查询参数
		}).flexReload();// 重新加载
	$("#vehicleOnlinePercert").show();
}
...
function toExportExl() {
	exportExl('vehicleOnlinePercertList', 'query/stat/vehicleOnlinePercertExportExl.action');
}
